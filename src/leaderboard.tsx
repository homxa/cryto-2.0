...
const Leaderboard = () => {
  ...
  const [leaderboardData, setLeaderboardData] = useState([
    { rank: 1, name: "John Doe", points: 1000, imageUrl: "https://via.placeholder.com/50" },
    ...
    <div className="bg-black text-white min-h-screen font-sans">
      ...
      <div className="container mx-auto p-8">
        <div className="flex items-center mb-8">
          <img src={currentUser.imageUrl} alt={currentUser.name} className="rounded-full w-10 h-10 mr-2" />
              ...
              {leaderboardData.map((user, index) => (
                <tr key={index} className={(index % 2 === 0) ? "bg-gray-700" : "bg-gray-800"}>
                  ...
                  <td className="px-6 py-4 flex items-center">
                    <img src={user.imageUrl} alt={user.name} className="rounded-full w-10 h-10 mr-2" />
                    <span>{user.name}</span>
                  </td>
